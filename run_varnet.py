...
import os
from data_utils import FastMRISpecificDataset
parser = argparse.ArgumentParser(description='Variational network arguments')
import torch
torch.set_float32_matmul_precision('high')  # 加入这一行
# Data IO
...
parser.add_argument('--sampling_pattern', type=str, default='cartesian', help='type of sampling pattern')

parser.add_argument('--data_dir', type=str, default='/home/liujunda/data_fastmri_train', help='fastmri data dir')
parser.add_argument('--smaps_dir', type=str, default='/home/liujunda/data_fastmri_train_smaps', help='smaps dir')
...
dataset = FastMRISpecificDataset(
    data_dir=args['data_dir'],
    smaps_dir=args['smaps_dir']
)
if args['mode'] == 'train':
	shuffle = True
else:
...
trainer = pl.Trainer(accelerator='gpu', devices=len(args['gpus'].split(',')), max_epochs=args['epoch'])
